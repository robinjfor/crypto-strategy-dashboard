...
def evaluate_news_burst_slot(slot: dict, klines: pd.DataFrame, position: dict | None, slot_meta: dict) -> dict:
    """news_burst_confirm: z>k + donchian/ema confirm. Fail-closed if news pipeline stale."""
    from indicators import add_donch_atr, split_closed, bar_ts_iso, replay_stop_path
    from news_ingest import aligned_news_z, news_pipeline_ok
    from news_lib import news_burst_confirm_signal
    ...
    ind = add_donch_atr(closed, donch_n, atr_mode=atr_mode).dropna(subset=["atr", "donch_hi", "donch_lo"])
    if ind.empty:
        return {"slot": slot["id"], "symbol": slot["symbol"], "error": "no_closed_bars", "family": "news_burst_confirm"}

    k = float(slot.get("news_k") or slot.get("k") or 1.5)
    field = str(slot.get("news_field") or slot.get("field") or "vol")
    confirm = str(slot.get("news_confirm") or slot.get("confirm") or "donchian").strip().lower()
    if confirm not in ("donchian", "ema"):
        confirm = "donchian"

    pipeline_ok = news_pipeline_ok()
    bar = ind.iloc[-1]
    bar_ts = bar_ts_iso(bar.name)
    atr = float(bar["atr"])
    mark = float(forming["Close"]) if forming is not None else float(bar["Close"])
    result = {
        "slot": slot["id"], "symbol": slot["symbol"], "tf": tf,
        "family": "news_burst_confirm", "atr_mode": atr_mode,
        "news_k": k, "news_field": field, "news_confirm": confirm,
        "news_pipeline_ok": pipeline_ok,
        "bar_ts": bar_ts, "mark": mark, "close": float(bar["Close"]),
        "donch_hi": float(bar["donch_hi"]), "donch_lo": float(bar["donch_lo"]),
        "atr": round(atr, 8), "side": "LONG", "venue": "spot",
        "checked_at": now_iso_taipei(), "action": "hold", "reason": None,
    }

    if position and position.get("status") == "FILLED":
        # Manage with ATR trail / donch_lo / max_hold like donchian; also exit if confirm signal goes flat
        base = evaluate_donchian_slot(slot, klines, position, slot_meta)
        base["family"] = "news_burst_confirm"
        base["news_pipeline_ok"] = pipeline_ok
        if pipeline_ok:
            sym = str(slot["symbol"]).replace("USDT", "")
            z_al = aligned_news_z(sym, field, ind.index)
            sig = news_burst_confirm_signal(ind, z_al, k, confirm)
            result["signal"] = int(sig.iloc[-1])
            if int(sig.iloc[-1]) == 0 and base.get("action") in ("manage", "hold"):
                base.update(action="exit", reason="news_confirm_off", exit_ref=float(bar["Close"]), exit_bar_ts=bar_ts)
        return base | {"family": "news_burst_confirm", "news_k": k, "news_field": field, "news_confirm": confirm, "news_pipeline_ok": pipeline_ok}
    ...
    if slot_meta.get("last_acted_bar_ts") == bar_ts:
        ...
        return result
    if not pipeline_ok:
        result.update(action="armed", reason="news_unavailable_no_entry")
        return result
    ...
    sig = news_burst_confirm_signal(ind, z_al, k, confirm)
    result["signal"] = int(sig.iloc[-1])
    result["news_z"] = None if pd.isna(z_al.iloc[-1]) else round(float(z_al.iloc[-1]), 4)
    if not _rising_edge(sig):
        result.update(action="armed", reason="waiting_news_burst_confirm")
        return result
    quote = min(float(slot["quote_usdt"]), MAX_NOTIONAL_USDT)
    suggested_stop = float(bar["Close"]) - float(slot.get("stop_atr_mult") or 1.5) * atr
    result.update(
        action="enter", reason="news_burst_confirm",
        quote_usdt=quote, suggested_stop=round(suggested_stop, 8),
        client_order_id=f"{slot['id']}-{bar_ts[:16].replace(':', '').replace('+', '')}"[:36],
    )
    return result
...
def evaluate_news_filter_slot(slot: dict, klines: pd.DataFrame, position: dict | None, slot_meta: dict) -> dict:
    """news_filter_donchian: Donchian entry only when news z>k. Fail-closed if news stale."""
    from indicators import add_donch_atr, signal_donchian_state, split_closed, bar_ts_iso, needs_reset_below_hi
    from news_ingest import aligned_news_z, news_pipeline_ok
    from news_lib import apply_entry_mask
    ...
    ind = add_donch_atr(closed, donch_n, atr_mode=atr_mode).dropna(subset=["atr", "donch_hi", "donch_lo"])
    if ind.empty:
        return {"slot": slot["id"], "symbol": slot["symbol"], "error": "no_closed_bars", "family": "news_filter_donchian"}
    ...
    if pipeline_ok:
        ...
        z_al = aligned_news_z(sym, field, ind.index)
        entry_ok = z_al > k
        sig = apply_entry_mask(raw, entry_ok)
        news_z = None if pd.isna(z_al.iloc[-1]) else round(float(z_al.iloc[-1]), 4)
    else:
    ...
    result = {
        "slot": slot["id"], "symbol": slot["symbol"], "tf": tf,
        "family": "news_filter_donchian", "atr_mode": atr_mode,
        "news_k": k, "news_field": field, "news_pipeline_ok": pipeline_ok,
        "news_z": news_z, "signal": int(sig.iloc[-1]), "raw_signal": int(raw.iloc[-1]),
        "bar_ts": bar_ts, "mark": mark, "close": float(bar["Close"]),
        "donch_hi": float(bar["donch_hi"]), "donch_lo": float(bar["donch_lo"]),
        "atr": round(atr, 8), "side": "LONG", "venue": "spot",
        "checked_at": now_iso_taipei(), "action": "hold", "reason": None,
    ...
    if position and position.get("status") == "FILLED":
        return evaluate_donchian_slot(slot, klines, position, slot_meta) | {
            "family": "news_filter_donchian", "news_k": k, "news_field": field,
            "news_pipeline_ok": pipeline_ok, "news_z": news_z,
        }
    ...
    if slot.get("require_reset_below_hi") and slot_meta.get("last_exit_bar_ts"):
        if needs_reset_below_hi(ind, slot_meta.get("last_exit_bar_ts")):
            ...
            return result
    if not _rising_edge(sig):
        result.update(action="armed", reason="waiting_breakout_or_news")
        return result
    quote = min(float(slot["quote_usdt"]), MAX_NOTIONAL_USDT)
    suggested_stop = float(bar["Close"]) - float(slot.get("stop_atr_mult") or 1.5) * atr
    result.update(
        action="enter", reason="donchian_breakout_news_filter",
        quote_usdt=quote, suggested_stop=round(suggested_stop, 8),
        client_order_id=f"{slot['id']}-{bar_ts[:16].replace(':', '').replace('+', '')}"[:36],
    )
    return result
...
def evaluate_slot_dispatch(slot: dict, klines: pd.DataFrame, position: dict | None, slot_meta: dict) -> dict:
    ...
    if fam == "news_burst_confirm":
        return evaluate_news_burst_slot(slot, klines, position, slot_meta)
    if fam == "news_filter_donchian":
        return evaluate_news_filter_slot(slot, klines, position, slot_meta)
    if fam == "donchian_lev":
        return {
